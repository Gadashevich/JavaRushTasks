...
public class Solution {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
        if(dateTime.getDayOfWeek().getValue() == FRIDAY && dateTime.getHour() >= WEEKEND_START_FRIDAY_CUT_OFF_HOUR) return true;
        if (dateTime.getDayOfWeek().getValue() ==SATURDAY) return true;
        if(dateTime.getDayOfWeek().getValue() == SUNDAY && dateTime.getHour()< WEEKEND_END_SUNDAY_CUT_OFF_HOUR) return  true;
        else {
        return false;
